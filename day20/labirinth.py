...
class Labirinth:
    ...
    def __init__(self):
        ...
        #how often print out progress -1 means disabled
        self.cn_iteration_debug = -1
    ...
    def get_map_cost(self) -> Map_of_symbols:
        return self.gcl_map_cost
    ...
    def load_from_file( self, is_filename : str ) -> bool:
        ...
        #Load Start
        b_fail, ltnn_start = self.gcl_map.find_symbol(self.cs_start)
        if b_fail or len(ltnn_start) != 1:
            logging.error(f"ERROR: Start not found")
            return True #FAIL
        else:
            tnn_start = ltnn_start[0]
            self.gtnn_start = tnn_start
            #replace symbol with void
            b_fail = self.gcl_map.set_coordinate( tnn_start, self.cs_void )
            logging.info(f"Start {self.gtnn_start}")

        #Load Goal End
        b_fail, ltnn_goal = self.gcl_map.find_symbol(self.cs_end)
        if b_fail or len(ltnn_goal) != 1:
            logging.error(f"ERROR: End not found")
            return True #FAIL
        else:
            tnn_goal = ltnn_goal[0]
            self.gtnn_end = tnn_goal
            #replace symbol with void
            b_fail = self.gcl_map.set_coordinate( tnn_goal, self.cs_void )
            logging.info(f"End {self.gtnn_end}")
    ...
    def find_shortest_path(self, ib_debug = False ) -> Tuple[bool, Set[Tuple[int,int]] ]:
        ...
        #ENDPOINTS
        itnn_start : Tuple[int, int] = self.gtnn_start
        itnn_end : Tuple[int, int] = self.gtnn_end
