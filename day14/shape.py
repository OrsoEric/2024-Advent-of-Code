...
class Shape:
    ...
    def find_shapes(self, iltnn_coordinates: List[Tuple[int, int]], ib_debug = False ) ->  List[Set[Tuple[int, int]]]:
        """
        From a list of coordinates
        Cluster all coordinates that are adjacents
        And return a list of list of coordinates
        Sorted by the biggest cluster to the smallest cluster
        """

        #put all the input coordinates in a set
        dtnn_coordinates = set()
        for tnn_coordinate in iltnn_coordinates:
            #i don't care about duplicates
            if tnn_coordinate not in dtnn_coordinates:
                dtnn_coordinates.add(tnn_coordinate)
        if ib_debug:
            logging.debug(f"Num: {len(dtnn_coordinates)} Coordinates: {dtnn_coordinates}")
        #cluster all adjacent coordinate
        ldtnn_adjacent = self.cluster_adjacent_coordinates( dtnn_coordinates )
        return ldtnn_adjacent
    ...
    def is_adjacent( self, itnn_a : Tuple[int, int], itnn_b : Tuple[int, int], b_debug = False) -> bool:
        ...
        if b_debug:
            logging.debug(f"A: {itnn_a} B: {itnn_b} Distance: {n_distance} | Adjacent: {n_distance == 1}")
        if n_distance == 1:
            return True
        else:
            return False
    ...
    def cluster_adjacent_coordinates(self, dtnn_coordinates : Set[Tuple[int, int]], ib_debug = False ) -> List[Set[Tuple[int, int]]]:
        """
        ...
        while b_continue:
            ltnn_adjacent = list()
            if ib_debug:
                logging.debug(f"Cluster: {dtnn_cluster} | Pool of Coordinates to be clustered: {dtnn_coordinates}")
            ...
            for tnn_cluster_coordinate in dtnn_cluster:
                ...
                for tnn_coordinate_to_be_clustered in dtnn_coordinates:
                    ...
                    if self.is_adjacent( tnn_cluster_coordinate, tnn_coordinate_to_be_clustered):
                        #make sure that I add a coordinate just once, even if it's surrounded by clustered coordinates
                        if tnn_coordinate_to_be_clustered not in ltnn_adjacent:
                            ltnn_adjacent.append(tnn_coordinate_to_be_clustered)
            if ib_debug:
                logging.debug(f"Adjacents fund: {len(ltnn_adjacent)} | {ltnn_adjacent} to cluster {dtnn_cluster}")
